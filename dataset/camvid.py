...
# 对11各类别进行分割。我们将图像调整为360*480像素，用于训练和测试#
...
# ----------------------------------------- #
#   args；参数
#   root：数据集的根目录，默认为空字符串
#   list_path: 数据集列表文件的路径，默认为空字符串。
#   max_iters: 数据集最大迭代次数，默认为None
#   crop_size: 图像裁剪大小，默认为(360, 360)。
#   mean: 图像RGB三通道均值，用于数据预处理，默认为(128, 128, 128)。
#   scale: 是否进行图像缩放，默认为True。
#   mirror: 是否进行图像镜像翻转，默认为True。
#   ignore_label: 忽略标签，用于标记不关心的类别，默认为11。
#   这个函数作用是初始化一个数据集对象，该对象用于读取和处理语义分割任务中的图像和标签数据
class CamVidDataSet(data.Dataset):
    ...
    # 下面代码的作用是根据指定的训练图像列表文件路径‘list_path’加载训练集，并根据‘max_iters’参数进行数据扩充
    ...
    # 作用：根据读取到的训练训练图像ID，获取每个图象的路径和标签，并将它们存储在‘self.files’列表中
